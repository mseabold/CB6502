 ...
 * (c) 2020 Matt Seabold
 *
 * Originally based on Make Chamber's Fake6502 (See below),
 * but with modifications for extra support:
 *
 * - WD65C02 specific opcodes
 * - General cleanup/modularization
 */
...
static read_callback_t read6502 = dummy_read;
static write_callback_t write6502 = dummy_write;
...
{
    read6502 = read_callback;
    write6502 = write_callback;
    reset6502();
}
...
{
    snprintf(buffer, bufLen, "0x%04x: 0x%02x", pc, read6502(pc));
}
